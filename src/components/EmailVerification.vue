<template lang="pug">
  div
    v-alert(v-if="!verified && !verificationResend && !verificationResendError" type="warning" color="caution" icon="fas fa-exclamation-triangle" value="true") {{ user.displayName }}, you have not verified your email yet.&nbsp;&nbsp;Please check your inbox (or spam or trash folder) for a email verification message.&nbsp;&nbsp;If you need to resend the verification email, #[a.blue--text.text--darken-4(@click="resendVerificationEmail") click here].
    v-alert(v-if="!verified && verificationResend" type="success" icon="fas fa-check" value="true") Your verification email has been resent.&nbsp;&nbsp;Please check your inbox (or spam or trash folder) for a email verification message.
