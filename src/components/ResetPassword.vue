<template lang="pug">
  v-dialog(v-model="resetPasswordModal" :width="width" persistent)
    form.pa-3.text-xs-center(@submit.prevent="submit" autocomplete="on")
      ...
      v-text-field#password(v-model="password"  placeholder="Password" :class="{'form-field--errors': areErrors('password')}" :error-messages="errors['password']" @input="validateField('password', 'Password', ['required', 'password'])" solo type="password")
      label.d-block.text-xs-left(for="password") Re-enter Password
      v-text-field#password(v-model="confirmPassword"  placeholder="Re-enter Password" :class="{'form-field--errors': areErrors('confirmPassword')}" :error-messages="errors['confirmPassword']" @input="validateField('confirmPassword', 'Re-Enter Password', ['required', 'sameAsPassword'])" solo type="password")
